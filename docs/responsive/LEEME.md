...
En los últimos años ha surgido un enfoque cada vez más **intrínseco y fluido** del diseño web adaptable. No se trata de abandonar por completo los _breakpoints_, sino de dejar que CSS haga más trabajo continuo por nosotros. Por ejemplo, la tipografía y los márgenes pueden definirse con `clamp()` y unidades relativas, reduciendo mucho las media queries necesarias[GitHub](https://github.com/ruvebal-udit/web-fundamentals/blob/eaa1f9b1d58060e616645f06d6f85d56b9e18a10/docs/responsive/LEEME.md#L401-L405). De igual modo, las **container queries** y las recién aparecidas **style queries** permiten que los componentes se adapten a su contexto inmediato.
...
- **Diseño Intrínseco (Intrinsic Design):** En este enfoque **el propio contenido conforma el layout**, en lugar de adaptarse pasivamente a él [harlointeractive.com](https://www.harlointeractive.com/blog/post/goodbye-responsive-hello-intrinsic#:~:text=The%20beauty%20of%20intrinsic%20design,it%20into%20rigidly%20proportioned%20columns). Con CSS Grid y Flexbox se crean diseños basados en el _tamaño intrínseco_ del contenido: áreas con contenido grande ocupan más espacio y las pequeñas se ajustan proporcionalmente[harlointeractive.com](https://www.harlointeractive.com/blog/post/goodbye-responsive-hello-intrinsic#:~:text=The%20beauty%20of%20intrinsic%20design,it%20into%20rigidly%20proportioned%20columns). Esto permite interfaces “elásticas” que minimizan hacks y media queries: por ejemplo, usando `grid-template-columns: minmax(min-content, 1fr)` se deja que las columnas crezcan según su contenido[harlointeractive.com](https://www.harlointeractive.com/blog/post/goodbye-responsive-hello-intrinsic#:~:text=The%20beauty%20of%20intrinsic%20design,it%20into%20rigidly%20proportioned%20columns). El diseño intrínseco busca que el layout sea más natural y **autoajustable**, reduciendo los saltos abruptos entre breakpoints.
...
> **Ejercicio 2:** _Tipografía fluida_ –  Introduce en tu `css` las siguientes variables y pruébalas luego con `font-size: var(--text-lg); line-height: var(--line-height-lg);` etc., logrando que los textos escalen automáticamente sin múltiples `@media`. El uso de `clamp()` facilita componentes “responsive por definición”.

```css
:root {
  /* ===== CSS FLUID TYPOGRAPHY SETUP BY @RUVEBAL ===== */
  /* ===== FONT SIZES ===== */
...
> Good designers and developers are innovative by nature, and as more and more content makers begin to play with intrinsic the design, the more stunning and jaw dropping their creations will become. — **Danny Decker** [https://www.harlointeractive.com/blog/post/goodbye-responsive-hello-intrinsic]()
